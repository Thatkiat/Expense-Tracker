...
function doPost(e) {
  try {
    ...
    if (action === 'renameProjectFolder')  return jsonOut(renameProjectFolder(payload));
    if (action === 'renameActivityFolder') return jsonOut(renameActivityFolder(payload));
...
function setupSheets(ss) {
  const defs = [
    ...
    {
      name: 'Activities',
      headers: ['id','projectId','projectName','code','name','budget','status'],
      widths: [120,120,160,80,220,100,80],
    },
    {
      name: 'Users',
...
function deleteProject(ss, id) {
  ...
  if (projSheet) {
    ...
    for (let i = pData.length - 1; i >= 1; i--) {
      if (String(pData[i][0]) === String(id)) { projSheet.deleteRow(i + 1); break; }
    }
  }
  // 2. ลบ activities ที่เป็นของโครงการนี้ (header-based)
  const actSheet = ss.getSheetByName('Activities');
  if (actSheet && actSheet.getLastRow() >= 2) {
    const aData    = actSheet.getDataRange().getValues();
    const aHeaders = aData[0];
    const iProjId  = aHeaders.indexOf('projectId');
    const col      = iProjId >= 0 ? iProjId : 1;   // fallback positional
    for (let i = aData.length - 1; i >= 1; i--) {
      if (String(aData[i][col]) === String(id)) actSheet.deleteRow(i + 1);
    }
  }
  // 3. soft-delete expenses ที่เป็นของโครงการนี้
  ...
  if (expSheet && expSheet.getLastRow() >= 2) {
    ...
    const iProj = eHeaders.indexOf('projectId');
    const iDel  = eHeaders.indexOf('deletedAt');
    const iUpd  = eHeaders.indexOf('updatedAt');
    const now   = new Date().toISOString();
    if (iProj >= 0) {
      for (let i = 1; i < eData.length; i++) {
        if (String(eData[i][iProj]) === String(id)) {
          if (iDel >= 0) expSheet.getRange(i+1, iDel+1).setValue(now);
          if (iUpd >= 0) expSheet.getRange(i+1, iUpd+1).setValue(now);
        }
      }
    }
  }
...
function deleteActivity(ss, id) {
  ...
  if (actSheet) {
    const aData = actSheet.getDataRange().getValues();
    for (let i = aData.length - 1; i >= 1; i--) {
      if (String(aData[i][0]) === String(id)) { actSheet.deleteRow(i + 1); break; }
    }
  }
  // 2. soft-delete expenses ที่เป็นของกิจกรรมนี้
  const expSheet = ss.getSheetByName('Expenses');
  if (expSheet && expSheet.getLastRow() >= 2) {
    const eData    = expSheet.getDataRange().getValues();
    const eHeaders = eData[0];
    const iAct = eHeaders.indexOf('activityId');
    const iDel = eHeaders.indexOf('deletedAt');
    const iUpd = eHeaders.indexOf('updatedAt');
    const now  = new Date().toISOString();
    if (iAct >= 0) {
      for (let i = 1; i < eData.length; i++) {
        const alreadyDeleted = iDel >= 0 && String(eData[i][iDel]||'').trim() !== '';
        if (!alreadyDeleted && String(eData[i][iAct]) === String(id)) {
          if (iDel >= 0) expSheet.getRange(i+1, iDel+1).setValue(now);
          if (iUpd >= 0) expSheet.getRange(i+1, iUpd+1).setValue(now);
        }
      }
...
function buildProjectFolderName(projectId, projectCode, projectName) {
  if (projectCode && projectName) return safeFolderName(projectCode + '_' + projectName);
  if (projectName) return safeFolderName(projectName);
  return safeFolderName(projectId || 'project');
}

function buildActivityFolderName(activityId, activityName, activityCode) {
  if (activityCode && activityName) return safeFolderName(activityCode + '_' + activityName);
  if (activityName) return safeFolderName(activityName);
  if (activityId)   return safeFolderName(activityId);
  return 'activity';
}
...
function moveFile(payload) {
  try {
    const { fileId, projectId, projectCode, projectName,
            ...
            expenseDate, fileType } = payload;
    const file       = DriveApp.getFileById(fileId);
    const rootFolder = getRootUploadFolder();
    const projFolder = getOrCreateSubFolder(rootFolder, buildProjectFolderName(projectId, projectCode, projectName));
    // เก็บตรง activity folder (ไม่มี subfolder เดือน)
    let targetFolder = projFolder;
    if (activityId || activityCode || activityName) {
      targetFolder = getOrCreateSubFolder(projFolder, buildActivityFolderName(activityId, activityName, activityCode));
    }
    // ── Rename ชื่อไฟล์: สร้างชื่อใหม่จาก date + projectCode ──
    const oldName = file.getName();
    // ดึง prefix (slip/rcpt) จากชื่อเดิม
    const prefix = oldName.startsWith('rcpt') ? 'rcpt' : 'slip';
    // ดึงนามสกุลไฟล์จากชื่อเดิม
    const dotIdx = oldName.lastIndexOf('.');
    const ext    = dotIdx >= 0 ? oldName.substring(dotIdx + 1).toLowerCase() : 'jpg';
    // สร้างชื่อใหม่
    const ts      = getTimestamp(expenseDate || '');
    const shortId = (projectCode || projectId || 'X').replace(/[^A-Za-z0-9]/g, '').substring(0, 8);
    const newName = prefix + '_' + ts + '_' + shortId + '.' + ext;
    file.setName(newName);

    // ── ย้าย folder ──
    const oldParents = file.getParents();
    const oldIds = [];
    while (oldParents.hasNext()) oldIds.push(oldParents.next().getId());
    targetFolder.addFile(file);
    oldIds.forEach(oldId => {
      if (oldId !== targetFolder.getId()) {
        try { DriveApp.getFolderById(oldId).removeFile(file); } catch(_) {}
      }
    });
    return { success: true, fileId: file.getId(), newName: newName };
  } catch (err) {
    return { success: false, error: err.toString() };
  }
}
...
function uploadImage(payload) {
  try {
    ...
    const rootFolder = getRootUploadFolder();
    const projFolder = getOrCreateSubFolder(rootFolder, buildProjectFolderName(projectId, projectCode, projectName));
    // เก็บตรง activity folder (ไม่มี subfolder เดือน — ชื่อไฟล์มีวันที่อยู่แล้ว)
    let targetFolder = projFolder;
    if (activityId || activityCode || activityName) {
      targetFolder = getOrCreateSubFolder(projFolder, buildActivityFolderName(activityId, activityName, activityCode));
    }
...
// ─────────────────────────────────────
function renameProjectFolder(payload) {
  try {
    const { projectId, projectCode, oldName, newName, newCode } = payload;
    const rootFolder = getRootUploadFolder();
    // หา folder เดิมด้วยชื่อเดิม
    const oldFolderName = safeFolderName(
      (projectCode || newCode) ? ((projectCode||newCode) + '_' + (oldName||newName)) : (oldName||newName)
    );
    // ลองหลายรูปแบบ: code_name, name เฉยๆ
    const candidates = [
      safeFolderName((projectCode||'') + '_' + (oldName||'')),
      safeFolderName(oldName||''),
      safeFolderName(projectId||''),
    ].filter(Boolean);

    let folder = null;
    for (const cand of candidates) {
      const iter = rootFolder.getFoldersByName(cand);
      if (iter.hasNext()) { folder = iter.next(); break; }
    }
    if (!folder) return { success: false, error: 'Project folder not found: ' + candidates.join(' | ') };

    const newFolderName = buildProjectFolderName(projectId, newCode || projectCode, newName);
    folder.setName(safeFolderName(newFolderName));
    return { success: true, oldName: folder.getName(), newName: newFolderName };
  } catch (err) {
    return { success: false, error: err.toString() };
  }
}

function renameActivityFolder(payload) {
  try {
    const { projectId, projectCode, projectName, activityId, oldName, newName, newCode } = payload;
    const rootFolder  = getRootUploadFolder();
    const projFolderName = buildProjectFolderName(projectId, projectCode, projectName);
    const projIter    = rootFolder.getFoldersByName(safeFolderName(projFolderName));
    if (!projIter.hasNext()) return { success: false, error: 'Project folder not found: ' + projFolderName };
    const projFolder  = projIter.next();

    // หา activity folder เดิม
    const candidates = [
      safeFolderName((oldName||'') + '_' + (activityId||'')),
      safeFolderName((payload.activityCode||'') + '_' + (oldName||'')),
      safeFolderName(oldName||''),
      safeFolderName(activityId||''),
    ].filter(Boolean);

    let actFolder = null;
    for (const cand of candidates) {
      const iter = projFolder.getFoldersByName(cand);
      if (iter.hasNext()) { actFolder = iter.next(); break; }
    }
    if (!actFolder) return { success: false, error: 'Activity folder not found: ' + candidates.join(' | ') };

    const newFolderName = buildActivityFolderName(activityId, newName, newCode || payload.activityCode);
    actFolder.setName(safeFolderName(newFolderName));
    return { success: true, newName: newFolderName };
  } catch (err) {
    return { success: false, error: err.toString() };
  }
}
...
function repairActivityHeaders() {
  // เพิ่มคอลัมน์ code ใน Activities sheet ถ้ายังไม่มี (migration สำหรับข้อมูลเก่า)
  const ss    = SpreadsheetApp.getActiveSpreadsheet();
  setupSheets(ss);   // setupSheets จะเพิ่ม columns ที่ขาดให้อัตโนมัติ
  const sheet = ss.getSheetByName('Activities');
  if (!sheet) { Logger.log('Activities sheet not found'); return; }
  const headers = sheet.getRange(1,1,1,sheet.getLastColumn()).getValues()[0];
  Logger.log('Activities headers after repair: ' + JSON.stringify(headers));
  Logger.log('code column index: ' + headers.indexOf('code'));
}
// ─────────────────────────────────────
